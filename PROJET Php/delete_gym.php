...
$sql_member="DELETE FROM member WHERE trainer_id=(select trainer_id from trainer  where gym_id=(select gym_id from gym where gym_id='$inf')))";
$sql_query_mem=mysqli_query($conn,$sql_member);

if ($sql_query_mem) {
	$sql_trainer="DELETE FROM trainer  where gym_id=(select gym_id from gym where gym_id='$inf'))";
